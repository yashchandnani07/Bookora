...
type SlotUpdatedHandler = (event: SlotUpdatedEvent) => void;
type OfferCreatedHandler = (event: OfferCreatedEvent) => void;

type BookingRealtimeHandlers =
  | SlotUpdatedHandler
  | {
      onSlotUpdated?: SlotUpdatedHandler;
      onOfferCreated?: OfferCreatedHandler;
    };
...
function normalizeSlotUpdate(value: Record<string, unknown>): SlotUpdatedEvent {
  return {
...
function normalizeOfferCreated(value: Record<string, unknown>): OfferCreatedEvent {
  return {
    offerId: String(value.offerId ?? value.OfferId ?? ""),
...
export function subscribeToBookingUpdates(handlers: BookingRealtimeHandlers) {
  const onSlotUpdated = typeof handlers === "function" ? handlers : handlers.onSlotUpdated;
  const onOfferCreated = typeof handlers === "function" ? undefined : handlers.onOfferCreated;
